...
async function main() {
  ...
  const badCreate = await api("/events", "POST", badCreatePayload, { expectFailure: true });
  ...
  const created = await api("/events", "POST", payload);
  ...
  const edited1 = await api(`/events/${created.id}/full`, "PATCH", updatePayload);
  ...
  const edited2 = await api(`/events/${created.id}/full`, "PATCH", updatePayload);
  ...
  const badUpdate = await api(`/events/${created.id}/full`, "PATCH", badUpdatePayload, { expectFailure: true });
