...
$ResultsDir = Join-Path $WebRoot "test-results\support-guided"
$ReportsDir = Join-Path $WebRoot "test-results\reports"

$SpecPath = Join-Path $TestsDir "support-all-roles-guided.visual.spec.ts"
...
function Backup-File {
  ...
  if (Test-Path -LiteralPath $Path) {
    ...
    $BackupPath = "$Path.bak-support-guided-tests-$Stamp"
...
Write-Host "[INFO] Criando teste visual guiado completo do suporte"

New-Item -ItemType Directory -Force -Path $TestsDir | Out-Null
New-Item -ItemType Directory -Force -Path $ResultsDir | Out-Null
New-Item -ItemType Directory -Force -Path $ReportsDir | Out-Null

Backup-File $SpecPath
...
Write-Host "[OK] Config Playwright guiada criada: $ConfigPath"

Write-Utf8NoBom -Path $SpecPath -Content $SpecContent
Write-Host "[OK] Spec visual guiada criada: $SpecPath"

Write-Utf8NoBom -Path $RunnerPath -Content $RunnerContent
Write-Host "[OK] Runner guiado criado: $RunnerPath"

Write-Utf8NoBom -Path $DebugRunnerPath -Content $DebugRunnerContent
Write-Host "[OK] Runner debug criado: $DebugRunnerPath"

Write-Host ""
Write-Host "[OK] Teste geral visual de suporte criado."
Write-Host ""
Write-Host "Rodar acompanhando o navegador:"
Write-Host "cd `"$ProjectRoot`""
Write-Host "powershell -ExecutionPolicy Bypass -File .\run-support-guided-visual-tests.ps1"
Write-Host ""
Write-Host "Rodar pausado passo a passo com Playwright Inspector:"
Write-Host "cd `"$ProjectRoot`""
