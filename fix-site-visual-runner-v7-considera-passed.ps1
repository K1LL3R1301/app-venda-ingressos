...
function Backup-File {
  ...
  if (Test-Path -LiteralPath $Path) {
    ...
    $BackupPath = "$Path.bak-site-visual-v7-$Stamp"
...
Write-Host "[INFO] Aplicando Runner visual V7: uma execucao, relatorio limpo e falso exit code corrigido"
...
$ReportPath = Join-Path $ReportDir "site-visual-report-v7-$ModeName-$Stamp.txt"
$SummaryPath = Join-Path $ReportDir "site-visual-summary-v7-$ModeName-$Stamp.txt"
$ServerLogPath = Join-Path $ReportDir "web-server-v7-$Stamp.log"
$HtmlReportDir = Join-Path $WebRoot "playwright-report"
$OutputDir = Join-Path $RunOutputRoot "$Stamp-site-all-visual"
$RunTimeoutSeconds = if ($Headed) { 240 } else { 180 }
...
function Has-FailureSummary {
  ...
  return [System.Text.RegularExpressions.Regex]::IsMatch(
    $Text,
    "\b\d+\s+failed\b|\b\d+\s+timed out\b|\b\d+\s+interrupted\b|Error:\s|FAIL\b|TimeoutError"
  )
}
...
$StartedServer = $null
$SpecsText = ""
$SpecOut = Join-Path $ReportDir "site-visual-v7-$Stamp.out.txt"
$SpecErr = Join-Path $ReportDir "site-visual-v7-$Stamp.err.txt"
...
try {
  ...
  Write-Host "[INFO] Projeto: $ProjectRoot"
  Write-Host "[INFO] Runner visual V7"
  Write-Host "[INFO] Modo: $ModeName"
  Write-Host "[INFO] Timeout geral da execucao: $RunTimeoutSeconds segundos"
  ...
  $SpecsText = $VisualSpecs -join "`r`n - "
  ...
  $env:ASTRO_PLAYWRIGHT_OUTPUT_DIR = $OutputDir
  ...
  try {
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host "[INFO] Rodando suite visual completa"
    Write-Host "[INFO] Output isolado: $OutputDir"
    ...
    $Completed = $Process.WaitForExit($RunTimeoutSeconds * 1000)

    if (!$Completed) {
      Write-Host "[ERRO] TIMEOUT na suite visual. Encerrando processo."
    ...
    if (Test-Path -LiteralPath $SpecOut) {
      ...
      Write-Host ""
      Write-Host "[SAIDA DO PLAYWRIGHT]"
      Write-Host $CleanOut
    }

    if (Test-Path -LiteralPath $SpecErr) {
      $CleanErr = Remove-ColorCodes -Text ([System.IO.File]::ReadAllText($SpecErr))
      if (![string]::IsNullOrWhiteSpace($CleanErr)) {
        Write-Host ""
        Write-Host "[ERROS DO PLAYWRIGHT]"
        Write-Host $CleanErr
      }
    }

    $Combined = "$CleanOut`n$CleanErr"

    if ($FinalExitCode -ne 0 -and (Has-PassedSummary -Text $Combined) -and !(Has-FailureSummary -Text $Combined)) {
      Write-Host ""
      Write-Host "[AVISO] Playwright retornou exit code $FinalExitCode, mas o resumo mostra todos os testes passando."
      Write-Host "[OK] Vou considerar como sucesso porque o log contem apenas resumo de passed."
      $FinalExitCode = 0
    }

    if ($FinalExitCode -eq 0) {
      Write-Host ""
      Write-Host "[OK] Suite visual passou."
    } elseif ($FinalExitCode -eq 124) {
      Write-Host ""
      Write-Host "[ERRO] Suite visual travou e foi encerrada por timeout."
    } else {
      Write-Host ""
      Write-Host "[ERRO] Suite visual falhou com exit code $FinalExitCode."
    }
  } finally {
...
Modo: $ModeName
Timeout geral: $RunTimeoutSeconds segundos
...
Saida bruta do Playwright:
$SpecOut

Erros brutos do Playwright:
$SpecErr
...
if (!$Audit -and $FinalExitCode -ne 0) {
...
Write-Host "[OK] Core runner V7 criado."
...
Write-Host ""
Write-Host "[OK] Runner V7 aplicado."
Write-Host ""
