...
$AdminSupportNewPath = Join-Path $WebRoot "src\app\admin\support\new\page.tsx"
...
function Backup-File {
  ...
  if (Test-Path -LiteralPath $Path) {
    ...
    $BackupPath = "$Path.bak-support-agenda-whatsapp-$Stamp"
...
Write-Host "[INFO] Criando suporte central com agenda + chat por evento"

Backup-File $AdminSupportPath
Backup-File $AdminSupportNewPath
                  ...
                  <div className="border-t border-slate-200 bg-white p-4">
                    <div className="flex gap-2">
                      <textarea
                        ...
                        placeholder={selectedClosed ? "Chamado resolvido. Reabra para enviar novas mensagens." : "Digite uma mensagem..."}
                        onKeyDown={(event) => {
...
Write-Utf8NoBom -Path $AdminSupportPath -Content $AdminSupportPage
Write-Host "[OK] /admin/support atualizado com agenda + chat."

Write-Utf8NoBom -Path $AdminSupportNewPath -Content $AdminSupportNewPage
Write-Host "[OK] /admin/support/new atualizado com evento vindo da agenda."
...
Write-Host ""
Write-Host "[OK] Central de suporte por agenda criada."
Write-Host ""
Write-Host "Agora rode:"
Write-Host "cd `"$WebRoot`""
Write-Host "npm run build *> log-web-support-agenda-whatsapp-build.txt"
Write-Host "Select-String -Path .\log-web-support-agenda-whatsapp-build.txt -Pattern `"error|Error:|Failed|Cannot find|Type error|Module not found|admin/support|admin/support/new|eventId|resolveSupportTicketReal|addSupportMessageReal|forwardSupportToSuperAdminReal`" -Context 2,3"
