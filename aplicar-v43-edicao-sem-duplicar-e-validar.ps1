# aplicar-v43-edicao-sem-duplicar-e-validar.ps1
# Corrige o PATCH /events/:id/full:
# - Quando o evento tem pedidos, nao cria datas/setores de novo.
# - Atualiza datas/setores existentes por ordem.
# - Cria somente datas/setores novos se o payload tiver mais itens que o banco.
# - Adiciona validacao de capacidade no update full.
#
...
$Backup = "$ControllerPath.bak-v43-$Stamp"
...
# -------------------------------------------------------------------
# 1) Adiciona BadRequestException no import, se ainda nao tiver.
# -------------------------------------------------------------------
$Text = $Text.Replace(
...
# -------------------------------------------------------------------
# 2) Adiciona helper de validacao de capacidade do update full.
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 3) Chama ensureFullUpdateCapacity(body) logo depois de confirmar existing.
# -------------------------------------------------------------------
...
'@
  ...
  $Replacement = @'
    ...
    ensureFullUpdateCapacity(body);

    const eventOccupancyMode = occupancy(body.occupancyMode) || existing.occupancyMode;
'@

  if (-not $Text.Contains($Needle)) {
    throw "Nao encontrei ponto para chamar ensureFullUpdateCapacity(body)."
  }
...
# -------------------------------------------------------------------
# 4) Substitui bloco de criacao de sessions/sectors por bloco seguro.
# -------------------------------------------------------------------
...
Write-Host "v43 aplicada com sucesso." -ForegroundColor Green
