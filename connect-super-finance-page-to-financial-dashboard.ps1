...
$FinanceDir = Join-Path $ProjectRoot "apps\web\src\app\admin\super\finance"
$PagePath = Join-Path $FinanceDir "page.tsx"
$ComponentPath = Join-Path $FinanceDir "FinancialDashboardPage.tsx"
...
Write-Host "[INFO] Ligando tela /admin/super/finance ao endpoint /dashboard/financial"

Backup-File -Path $PagePath -Tag "connect-financial-dashboard"
Backup-File -Path $ComponentPath -Tag "connect-financial-dashboard"
...
Write-Host "[OK] page.tsx atualizado."
Write-Host "[OK] FinancialDashboardPage.tsx criado."

Set-Location $WebRoot
Write-Host "[INFO] Limpando cache .next..."
if (Test-Path -LiteralPath ".next") {
  Remove-Item -Recurse -Force ".next"
}

Write-Host "[INFO] Rodando build da web..."
npm run build *> log-web-connect-financial-dashboard-build.txt

$BuildLogPath = Join-Path $WebRoot "log-web-connect-financial-dashboard-build.txt"
$Errors = Select-String -Path $BuildLogPath -Pattern "error|Error:|Failed|Cannot find|Type error|Module not found|financial|finance|dashboard|Expected|Duplicate" -Context 2,3
...
Write-Host ""
Write-Host "[OK] Tela financeira conectada."
Write-Host ""
