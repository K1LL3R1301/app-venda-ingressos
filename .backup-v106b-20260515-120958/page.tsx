...
export default function CustomerOrderDetailPage() {
  ...
  async function loadTicketQrToken(ticket: TicketItem) {
    ...
    setSelectedTicketQrToken("");
    setSelectedTicketQrError("");
    setSelectedTicketQrLoading(true);
      ...
      if (!res.ok || !data.token) {
        setSelectedTicketQrError(
          typeof data?.message === "string"
            ? data.message
            : "Não foi possível gerar o QR Code seguro deste ingresso.",
        );
        return;
      }

      setSelectedTicketQrToken(data.token);
    } catch (error) {
      console.error("LOAD TICKET QR TOKEN ERROR:", error);
