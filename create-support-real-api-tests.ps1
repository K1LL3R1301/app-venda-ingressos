...
$ScriptPath = Join-Path $ProjectRoot "run-support-real-api-tests.ps1"
...
if (Test-Path -LiteralPath $ScriptPath) {
  ...
  $BackupPath = "$ScriptPath.bak-real-api-tests-$Stamp"
...
Write-Utf8NoBom -Path $ScriptPath -Content $Runner

Write-Host "[OK] Script criado: $ScriptPath"
Write-Host ""
Write-Host "Antes de rodar, deixe a API ligada em outro terminal:"
Write-Host "cd `"$ProjectRoot\apps\api`""
Write-Host "npm run start:dev"
Write-Host ""
Write-Host "Depois rode o teste real:"
