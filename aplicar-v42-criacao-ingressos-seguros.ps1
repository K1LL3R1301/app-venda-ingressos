# aplicar-v42-criacao-ingressos-seguros.ps1
# Aplica travas de capacidade na CRIACAO do evento.
# Rode na raiz do projeto: plataforma-ingressos
#
# O que faz:
# - Front /admin/events/new: antes de enviar para API, corta qualquer excesso de lotes por data + setor.
# - API events.service.ts: recusa salvar criacao se a soma dos ingressos ultrapassar capacidade geral/data/setor.
#
# Mantem backup automatico dos arquivos alterados.
...
'@
  ...
  $Original1 = "ticketTypes: tickets.map((ticket, index) => ({"
  $Replacement1 = "ticketTypes: clampTicketsForCapacity(tickets, sessions, sectors, capacity, isOpenOnly).map((ticket, index) => ({"

  if ($Text.Contains($Original1)) {
    $Text = $Text.Replace($Original1, $Replacement1)
    Write-Host "Payload de criacao agora usa tickets saneados por capacidade." -ForegroundColor Green
  }
  elseif ($Text.Contains($Replacement1)) {
    Write-Host "Payload de criacao ja estava usando clampTicketsForCapacity." -ForegroundColor Yellow
  }
  else {
    throw "Nao encontrei o trecho 'ticketTypes: tickets.map'. Patch do front incompleto."
  }
...
'@
  ...
  elseif ($Text.Contains("this.ensureTicketCapacityForCreate(data, sessionsPayload, sectorsPayload);")) {
...
Write-Host "Aplicando v42 - criacao segura de ingressos..." -ForegroundColor Cyan
Patch-NewPage
Patch-EventsService
Write-Host "v42 aplicada. Reinicie a API e o WEB." -ForegroundColor Green
