...
function mediaUrl(order?: AnyItem) { const media = order?.event?.media || {}; return media.bannerImageUrl || media.coverImageUrl || media.mobileBannerUrl || media.thumbnailUrl || ""; }
function safeJson(value: string) { try { return JSON.parse(value); } catch { return {}; } }
function getTicketDate(order?: AnyItem, ticket?: AnyItem) { return ticket?.eventSession?.startDate || ticket?.session?.startDate || order?.event?.startDate || order?.event?.eventDate || order?.createdAt; }
function ticketCode(ticket?: AnyItem) { return ticket?.qrToken || ticket?.secureToken || ticket?.code || ticket?.id || ""; }
function ticketPlace(ticket?: AnyItem) { const meta = typeof ticket?.accessMetadata === "string" ? safeJson(ticket.accessMetadata) : ticket?.accessMetadata || {}; return ticket?.accessLabel || meta.label || meta.chairLabel || ""; }
