...
function previewText(value?: string, max = 110) {
  if (!value) return "Descubra os detalhes deste evento e acompanhe novidades na plataforma.";
...
export default function CustomerDashboardPage() {
  ...
  const mostPurchasedSection = buildSection(filteredEvents.length ? filteredEvents : events, 0, 4);
  const todaySection = buildSection(filteredEvents.length ? filteredEvents : events, 2, 4);
  const cultureSection = buildSection(filteredEvents.length ? filteredEvents : events, 4, 4);
        ...
        {featuredEvent ? (
                  ...
                  <div className="relative z-10 flex h-full flex-col justify-between">
                    ...
                    <div className="relative z-10 mt-10 max-w-3xl">
                      ...
                      <div className="mt-6 flex flex-wrap gap-4 text-sm text-white/85">
                        ...
                        <span>
                          👥 Capacidade: {featuredEvent.capacity ?? 0}
                        </span>
                        <span>
                          🏷️ {featuredEvent.status || "Ativo"}
                        </span>
                      </div>
                      ...
                      <div className="mt-8 flex flex-wrap gap-3">
                        <button
                          type="button"
                          onClick={() =>
                            goTo(`/customer/events/${featuredEvent.id}`)
                          }
          ...
          <section className="rounded-[32px] border border-dashed border-gray-300 bg-white p-10 text-center shadow-sm">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-gray-500">
              Ainda não existem eventos cadastrados para exibir nesta home.
              Assim que você criar eventos no painel, eles aparecerão aqui.
            </p>
            <button
              type="button"
            ...
            <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
              <div>
                ...
                <p className="mt-4 max-w-2xl text-sm leading-7 text-white/85">
                  Quando houver cancelamento com retorno em carteira, o valor
                  pode entrar na sua wallet e depois ser usado nas próximas
                  compras.
                </p>
