...
export default function CustomerOrdersPage() {
  ...
  async function reloadOrdersPageData() {
  ...
  async function handleAcceptTransferFromOrders(transferRequestId: string) {
    ...
    setTransferProcessingId(transferRequestId);
      ...
      if (!res.ok) {
        alert(
          typeof data?.message === "string"
            ? data.message
            : "Erro ao aceitar transfer̻ncia",
        );
        return;
      }

      alert("Transfer̻ncia aceita com sucesso.");
      await reloadOrdersPageData();
    } catch (error) {
      console.error("ACCEPT TRANSFER FROM ORDERS ERROR:", error);
      alert("Erro ao conectar com a API");
    } finally {
  ...
  async function handleRejectTransferFromOrders(transferRequestId: string) {
    ...
    const reason = window.prompt("Motivo da recusa (opcional)", "Transfer̻ncia recusada");
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/v1/tickets/customer/transfers/${transferRequestId}/reject`,
        {
          ...
          body: JSON.stringify({
            reason: reason || "Transfer̻ncia recusada",
          }),
        },
      );
      ...
      if (!res.ok) {
        alert(
          typeof data?.message === "string"
            ? data.message
            : "Erro ao recusar transfer̻ncia",
        );
        return;
      }

      alert("Transfer̻ncia recusada.");
      await reloadOrdersPageData();
    } catch (error) {
      console.error("REJECT TRANSFER FROM ORDERS ERROR:", error);
      alert("Erro ao conectar com a API");
    } finally {
      setTransferProcessingId(null);
    }
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    {String(transfer.status || "").toUpperCase() === "PENDING_ACCEPTANCE" ? (
                        <div className="mt-6 grid grid-cols-2 gap-3">
                          <button
                            type="button"
                            onClick={() => handleRejectTransferFromOrders(transfer.id)}
                            disabled={transferProcessingId === transfer.id}
                            className="flex items-center justify-center rounded-full border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60"
                          >
                            {transferProcessingId === transfer.id ? "..." : "Recusar"}
                          </button>

                          <button
                            type="button"
                            onClick={() => handleAcceptTransferFromOrders(transfer.id)}
                            disabled={transferProcessingId === transfer.id}
                            className="flex items-center justify-center rounded-full bg-[#ff6900] px-4 py-3 text-sm font-bold text-white hover:bg-[#e85f00] disabled:cursor-not-allowed disabled:opacity-60"
                          >
                            {transferProcessingId === transfer.id ? "..." : "Aceitar"}
                          </button>
                        </div>
                      ) : (
                        <button
                          type="button"
                          onClick={() => goTo(`/orders/${routeId}`)}
                          className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-[#242424] px-4 py-3 text-sm font-bold text-white hover:bg-black"
                        >
                          <span className="h-4 w-6 rounded-sm bg-gradient-to-r from-sky-400 via-amber-300 to-rose-400" />
                          Ver detalhes
